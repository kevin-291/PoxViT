...
class SkinDiseaseVisionTransformer(nn.Module):
    def __init__(self, img_size=IMG_SIZE, patch_size=PATCH_SIZE, num_classes=NUM_CLASSES, dim=768, depth=12, heads=12, mlp_dim=3072):
...
# Main function for training, validating, and testing
def main(data_dir):
...
# Load and use the model for inference (example)
def inference(data_dir):
    logging.info("Loading data for inference...")
    _, _, test_loader = load_data(data_dir)
    
    model = SkinDiseaseVisionTransformer().to(DEVICE)
    load_model(model, "skin_disease_vit.pth")  # Load the saved model
    
    logging.info("Starting inference...")
    accuracy = test(model, test_loader)
    logging.info(f"Inference Test Accuracy: {accuracy:.2f}%")
